...
fileStream.on("open", () => {
  s3.upload({
    ...
    Key: "Bazecor/" + process.env['TRAVIS_BUILD_NUMBER'] + "/" + fileName,
    Body: fileStream,
    ...
    if (data) {
      s3.putObject({
        Bucket: process.env['ARTIFACTS_BUCKET'],
        Key: "Bazecor/latest/Bazecor." + extensions[process.env['TRAVIS_OS_NAME']],
  ...
  s3.upload({
    Bucket: process.env['ARTIFACTS_BUCKET'],
    Key: "Bazecor/" + process.env['TRAVIS_BUILD_NUMBER'] + "/version.txt",
